...
docManager.prototype.createDemo = function (isSample, fileExt, userid, username, wopi) {
    console.log("crete a doc")
    const demoName = (isSample ? "sample" : "new") + "." + fileExt;
    console.log("createDemo - demoName = ",demoName)
...
// };
docManager.prototype.getServerUrl = function (forDocumentServer) {
    if (forDocumentServer && !!configServer.get("exampleUrl")) {
        console.log("example url is used")
      return configServer.get("exampleUrl");
    } else {
        console.log("exampleurl is not used");
...
docManager.prototype.storageRootPath = function (userAddress) {
    console.log("storgaerootpath = ", path.join(storageConfigFolder, this.curUserHostAddress(userAddress)));
...
docManager.prototype.historyPath = function (fileName, userAddress, create) {
    ...
    console.log("directory (docmanager historypath) = ",directory)
    return directory;
};
...
// get all the stored files (USED API's are /docManager, )
docManager.prototype.getStoredFiles = function (courseName) {
    // const userAddress = this.curUserHostAddress("course_01");

    const userAddress = courseName
...
// get current user host address (USED API's are /docManager, )
docManager.prototype.curUserHostAddress = function (userAddress) {
    // console.log("userAddress = ",userAddress);
    if (!userAddress){
        console.log("useraddress is not passed")
        // console.log("x-header = ",this.req.headers["x-forwarded-for"] );
        // console.log("remoteAddress = ", this.req.connection.remoteAddress);
        ...
        // console.log("useradd in docmanager = ",userAddress);
        if(userAddress === '::1'){
            console.log("true...")
            userAddress = '::ffff:127.0.0.1';
        }
        return userAddress.replace(new RegExp("[^0-9a-zA-Z.=]", "g"), "_");

    } // if user address isn't passed to the function
        else{
            console.log("useraddress is passed")
            console.log(userAddress)
            return userAddress;
        }
        // console.log("reg replace  = ", userAddress.replace(new RegExp("[^0-9a-zA-Z.=]", "g"), "_"))
};
