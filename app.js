...
app.use(cors());
app.use(express.static('public')); // Serves files from the 'public' folder in your project's root directory
...
app.get('/courses', function(req, res) {
    ...
    try {
      req.docManager = new docManager(req, res);
      const course_path = configServer.get("storageFolder");
      
      let courses_Array = fileSystem.readdirSync(course_path)
        .filter(file => fileSystem.statSync(path.join(course_path, file)).isDirectory());
      
      console.log("courses_Array = ", courses_Array); // Log courses_Array, not courses
  
      res.render('courselist', {
        courses_Array: courses_Array // Correct variable name
      });
    } catch (e) {
      console.log(e);
    }
...
// to get the list of doc in a course 
app.get("/courses/:courseName", function (req, res) {  // define a handler for default page
    try {
        ...
        res.render("index", {  // render index template with the parameters specified
            ...
            storedFiles: req.docManager.getStoredFiles(courseName),
            params: req.docManager.getCustomParams(),
            users: users,
            serverUrl: req.docManager.getServerUrl(true),
...
app.post("/courses/:courseName/upload", function (req, res) {  // define a handler for uploading files

    console.log("uploading a doc ..................");
    req.docManager = new docManager(req, res);
    ...
    const uploadDir = req.docManager.storageRootPath(courseName);
    console.log("iploadDir = ",uploadDir);
...
app.post("/create", function (req, res) {
    console.log("document creations")
    ...
    try {
        ...
        var fileName = req.docManager.getCorrectName(title); // this is to generate a file name like new (1).docx
        var userAddress = req.docManager.curUserHostAddress();
        console.log("useradd = ",userAddress)
...
app.delete("/courses/file", function (req, res) {  // define a handler for removing file
    console.log("remove.......");
    try {
        console.log("remove.......");
    	req.docManager = new docManager(req, res);
...
//         updateFile(res, req.body, "/var/lib/docker/volumes/onlyoffice_docx_volume/_data/__ffff_127.0.0.1/yokesh_question1_student.docx");
//     } else {
//         readbody(req, res, "/var/lib/docker/volumes/onlyoffice_docx_volume/_data/__ffff_127.0.0.1/yokesh_question1_student.docx")
//     }
// });
    ...
    var processTrack = async function (response, body, fileName, userAddress) {
        ...
        var callbackProcessSave = async function (downloadUri, body, fileName, userAddress, newFileName) {
            try {
                ...
                console.log("track-storagepath = ",storagePath)
...
app.get("/editor", function (req, res) {  // define a handler for editing document
    ...
    try {
        ...
        // Done by yokesh on sep 13, 2023 .............

        console.log("req = ",req.query.mode);
        ...
        console.log("userDirectUrl",userDirectUrl,req.query.directUrl);
        var userid = user.id;
        var name = user.name;
        ...
        if (fileExt != null) {
            console.log("fileExit is not null")
        ...
        if (!req.docManager.existsSync(req.docManager.storagePath(fileName, userAddress))) {  // if the file with a given name doesn't exist
            ...
            if(fileExt === '.docx' || fileExt === '.xlsx' || fileExt === '.pptx'){
                ...
                // Create an empty file with the specified filename and path
                fileSystem.writeFile(filepath, '', (err) => {
                    if (err) {
                    console.error(`Error creating the file: ${err}`);
                    responseSent = false;
                    } else {
                    console.log(`Empty file "${filepath}" has been created successfully.`);
                    responseSent = true;
                    }
                });

                if(responseSent == true){
                    let redirecturl = req.docManager.getServerUrl() + "/editor?fileName=" + encodeURIComponent(fileName)+ req.docManager.getCustomParams();
            ...
            // const doc = officegen(fileExt);
            //     console.log("doc",doc)
            //  // Save the document to the specified file path
            // const stream = fs.createWriteStream(filepath + extension);
            // doc.generate(stream);
            // console.log(`File '${fileName}${extension}' created in directory '${userAddress}'`);
            // }
        ...
        console.log("url = ", url, "directUrl = ",directUrl);
        ...
        var argss = {
            ...
            editor: {
                ...
                protect: false,
                // print: false,
                download: req.query.mode === "embedded"? false : !user.deniedPermissions.includes("download"),
                // download: false,
                // print: !user.deniedPermissions.includes("print"),
                print: false,
                mode: mode != "view" ? "edit" : "view",
                ...
                instanceId: userid != "uid-0" ? req.docManager.getInstanceId() : null,
                logo: { 
                    image: "https://www.bmw.com/content/dam/bmw/marketBMWCOM/bmw_com/categories/automotive-life/explained-m/exlm-og.jpg", 
                    imageDark: "https://www.bmw.com/content/dam/bmw/marketBMWCOM/bmw_com/categories/automotive-life/explained-m/exlm-og.jpg", 
                    url: "https://platform.vidh.ai" 
                }
            },
        ...
        if (cfgSignatureEnable) {
            app.render('config', argss, function(err, html){  // render a config template with the parameters specified
                if (err) {
                 ...
                 // The token needs to be included in the context when rendering the template
                // argss.editor.token = argss.editor.token || ''; // Ensure the token is defined even if not signed
                console.log(argss)
